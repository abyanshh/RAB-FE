...
import { getAllAppointments } from '../../services/consult';
...
export default function ScheduleList( ) {
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const res = await axios.patch(
        `${import.meta.env.VITE_BASE_URL}/appointments/${selectedAppointment.appointment_id}/status`,
        { status: newStatus },
        {
          headers: {
  ...
  const totalPages = Math.ceil(filteredAppointments.length / dataPerPage);
          ...
          <tbody>
            {currentAppointments.length === 0 ? (
              ...
              currentAppointments.map((appointment) => (
                <tr key={appointment.appointment_id} className="border-t border-gray-300 hover:bg-cyan-50">
                    ...
                    <Button
                      ...
                      className={`rounded-md px-4 py-2 text-sm bg-white w-28 ${
                        appointment.status === 'Menunggu'
                          ...
                          : appointment.status === 'Selesai'|| appointment.status === 'Diterima'
